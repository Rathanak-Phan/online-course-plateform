...
@section('content')
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <h1 class="text-3xl font-extrabold text-slate-900 mb-8">Create New Course</h1>

        <form action="{{ route('instructor.courses.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8 bg-white p-10 rounded-2xl border border-slate-200 shadow-sm">
            @csrf

            <div>
                <x-input-label for="title" :value="__('Course Title')" />
                <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title')" required autofocus placeholder="e.g. Mastering Web Development" />
            ...
            <div class="grid grid-cols-2 gap-8">
                <div>
                    ...
                    <x-text-input id="price" class="block mt-1 w-full" type="number" name="price" :value="old('price', 0.00)" step="0.01" required />
            ...
            <div class="pt-4 border-t border-slate-100 flex justify-end gap-4">
                ...
                <button type="submit" class="bg-indigo-600 text-white px-10 py-4 rounded-xl font-bold hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-200">Create Course</button>
            </div>
        </form>
    </div>
@endsection
